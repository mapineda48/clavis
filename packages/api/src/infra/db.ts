...
export interface Db {
  ...
  /** Ends the pool. Idempotent enough for shutdown: failures are swallowed. */
  close(): Promise<void>
}
 ...
 */
async function waitForDatabase(pool: PgPool, log: Logger): Promise<void> {
...
/**
 * Creates the PostgreSQL pool, runs the pending migrations and returns the
 * database service. Everything that touches the database receives this object
 * (or a transaction client it handed out) as an argument — see `lib/executor.ts`.
 */
export async function createDb(options: DbOptions, log: Logger): Promise<Db> {
  const pool = new Pool({
    ...
    // requests that simply never answer. `waitForDatabase` above borrows it
  ...
  pool.on('error', (error) => {
    log.error({ err: error }, 'Unexpected error on a PostgreSQL pool client')
  })
  ...
  await runMigrations({ connectionString: options.DATABASE_URL }, log)
  ...
  return {
    ...
    async tx(fn) {
      ...
      const onError = (error: Error): void => {
        log.error({ err: error }, 'PostgreSQL connection lost during a transaction')
      }
      ...
      try {
        ...
        await client.query('ROLLBACK').catch((failure: unknown) => {
          ...
          log.error({ err: failure }, 'ROLLBACK failed; discarding the connection')
        })
        throw error
      } finally {
    ...
    async ping() {
      try {
        ...
        log.warn({ err: error }, 'PostgreSQL is not responding')
        return false
      }
    },
