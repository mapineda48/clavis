 ...
 *
 *   `db`      — one statement, its own implicit transaction
 *   `client`  — inside the `db.tx()` the caller already opened
 *
 ...
 *
 * `db.tx()` stays the unit of work; there is no abstraction on top of it.
 *
