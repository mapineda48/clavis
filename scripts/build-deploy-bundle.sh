...
#     secrets/cf_dns_token      Cloudflare token for the DNS-01 challenge
#     traefik/traefik.yml       rendered
#     traefik/dynamic/stack.yml rendered
#     realm/realm-erp.json      rendered
...
require ERP_APP_FQDN ERP_AUTH_FQDN ERP_ACME_EMAIL ERP_CERT_RESOLVER DEPLOY_SHA \
  ERP_API_IMAGE ERP_APP_IMAGE ERP_KEYCLOAK_IMAGE \
  ERP_DATABASE_URL ERP_STORAGE_CONNECTION_STRING ERP_ATTACHMENTS_CONTAINER \
  CLOUDFLARE_API_TOKEN KC_BOOTSTRAP_ADMIN_USERNAME KC_BOOTSTRAP_ADMIN_PASSWORD \
...
rm -rf dist
STAGE=dist/stage
mkdir -p "$STAGE/traefik/dynamic" "$STAGE/realm" "$STAGE/secrets"

REALM="${ERP_REALM:-erp}"
APP_URL="https://${ERP_APP_FQDN}"
AUTH_URL="https://${ERP_AUTH_FQDN}"
...
# databases.
DB_USER="$(printf '%s' "$ERP_DATABASE_URL" | sed -E 's|^postgres(ql)?://([^:]+):.*|\2|')"
DB_PASSWORD="$(printf '%s' "$ERP_DATABASE_URL" | sed -E 's|^postgres(ql)?://[^:]+:([^@]+)@.*|\2|')"
DB_HOST="$(printf '%s' "$ERP_DATABASE_URL" | sed -E 's|^postgres(ql)?://[^@]+@([^/]+)/.*|\2|')"
[ -n "$DB_USER" ] && [ -n "$DB_PASSWORD" ] && [ -n "$DB_HOST" ] || {
  echo "could not parse ERP_DATABASE_URL" >&2; exit 1; }

# --- realm --------------------------------------------------------------------
echo "==> rendering the realm"
OUTPUT_DIR="$STAGE/realm" \
KEYCLOAK_REALM="$REALM" \
KEYCLOAK_APP_CLIENT_ID="${KEYCLOAK_APP_CLIENT_ID:-erp-app}" \
KEYCLOAK_API_CLIENT_ID="${KEYCLOAK_API_CLIENT_ID:-erp-api}" \
KEYCLOAK_API_CLIENT_SECRET="$KEYCLOAK_API_CLIENT_SECRET" \
KEYCLOAK_PUBLIC_URL="$AUTH_URL" \
KEYCLOAK_LOGIN_THEME="${KEYCLOAK_LOGIN_THEME:-erp}" \
KEYCLOAK_EMAIL_THEME="${KEYCLOAK_EMAIL_THEME:-erp}" \
...
KEYCLOAK_SMTP_FROM_DISPLAY_NAME="${KEYCLOAK_SMTP_FROM_DISPLAY_NAME:-ERP Demo}" \
...
DEMO_MANAGER_EMAIL="${DEMO_MANAGER_EMAIL:-manager@erp.local}" \
DEMO_MANAGER_PASSWORD="$DEMO_MANAGER_PASSWORD" \
DEMO_USER_USERNAME="${DEMO_USER_USERNAME:-worker}" \
DEMO_USER_EMAIL="${DEMO_USER_EMAIL:-worker@erp.local}" \
DEMO_USER_PASSWORD="$DEMO_USER_PASSWORD" \
  node infra/keycloak/render-realm.mjs >/dev/null

[ -s "$STAGE/realm/realm-erp.json" ] || { echo "the realm did not render" >&2; exit 1; }
...
ADMIN_LIST=$(
  if [ -z "${ERP_ADMIN_ALLOWLIST_RAW:-}" ]; then echo '"127.0.0.1/32"'
  else printf '%s' "$ERP_ADMIN_ALLOWLIST_RAW" | tr ',' '\n' | sed 's/^ *//; s/ *$//' \
       | grep -v '^$' | sed 's/.*/"&"/' | paste -sd, -
  fi
)
export ERP_ADMIN_ALLOWLIST="$ADMIN_LIST" ERP_REALM="$REALM"

envsubst '${ERP_APP_FQDN} ${ERP_AUTH_FQDN} ${ERP_ACME_EMAIL} ${ERP_CERT_RESOLVER}' \
  < infra/deploy/traefik/traefik.yml.tpl > "$STAGE/traefik/traefik.yml"
envsubst '${ERP_APP_FQDN} ${ERP_AUTH_FQDN} ${ERP_ADMIN_ALLOWLIST} ${ERP_REALM}' \
...
printf '%s' "$CLOUDFLARE_API_TOKEN" > "$STAGE/secrets/cf_dns_token"
chmod 600 "$STAGE/secrets/cf_dns_token"
...
ERP_KEYCLOAK_IMAGE=${ERP_KEYCLOAK_IMAGE}
...
ERP_DB_HOST=${DB_HOST}
ERP_DB_USER=${DB_USER}
ERP_DB_PASSWORD=${DB_PASSWORD}
ERP_KC_DB_NAME=${ERP_KC_DB_NAME:-keycloak}

KC_BOOTSTRAP_ADMIN_USERNAME=${KC_BOOTSTRAP_ADMIN_USERNAME}
KC_BOOTSTRAP_ADMIN_PASSWORD=${KC_BOOTSTRAP_ADMIN_PASSWORD}

ERP_STORAGE_CONNECTION_STRING=${ERP_STORAGE_CONNECTION_STRING}
...
# /opt/erp/stack at 0700 root-owned, so nothing here is reachable by a non-root
# user on the droplet.
find "$STAGE" -type d -exec chmod 0755 {} +
find "$STAGE" -type f -exec chmod 0644 {} +
# The two that really are secrets, and that only root or the daemon reads.
chmod 0600 "$STAGE/.env" "$STAGE/secrets/cf_dns_token"
